...
The default `.env` uses `LLM_PROVIDER=heuristic`, so the demo works with no API key or paid model calls.
...
- Optional remote assessment is capped at `MAX_ASSESSMENT_CALLS` per comparison (10 by default); remaining items receive an explicitly labelled heuristic budget fallback. The evaluation endpoint always uses the local heuristic provider.
...
To use OpenAI, set `LLM_PROVIDER=openai`, `OPENAI_API_KEY`, and optionally `OPENAI_MODEL` in `.env`. The code falls back safely if the provider is unavailable or the per-comparison remote-call budget is exhausted. Keep `MAX_ASSESSMENT_CALLS` low for demos (the default is `10`).
